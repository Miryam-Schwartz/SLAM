...
class BundleAdjustment:
    ...
    def _init_bundle_windows(self):
        bundle_windows = dict()
        for first_keyframe in range(0, self._frames_num, self._window_len):
            last_key_frame = min(first_keyframe + self._window_len, self._frames_num - 1)
    ...
    def get_global_keyframes_poses(self):
        ...
        for first_keyframe, window in self._bundle_windows.items():
            last_keyframe = min(first_keyframe + self._window_len, self._frames_num - 1)
    ...
    def get_global_3d_points(self, global_poses):
        ...
        for window in self._bundle_windows.values():
            ...
            for pt in points_of_window:
                ...
                points_list.append([pt_global.x(), pt_global.y(), pt_global.z()])
