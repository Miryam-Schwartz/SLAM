...
def present_tracking_statistics(database):
    ...
    print(f"Mean track length: {round(mean, 3)}\nMaximum track length: {max_t}\nMinimum track length: {min_t}")
    print("Mean number of tracks on average frame: ", round(database.get_mean_number_of_tracks_on_frame(), 3))
    plot_number_of_matches_per_frame_graph(database)
...
def plot_number_of_matches_per_frame_graph(database):
    frames_num = database.get_frames_number()
    matches_number = np.empty(frames_num)
    for i in range(frames_num):
        matches_number[i] = database.get_frame_obj(i).get_kp_len()
    fig = px.line(x=np.arange(frames_num), y=matches_number, title="Number of matches per frame",
                  labels={'x': 'Frame', 'y': 'Number of matches'})
    fig.write_image(f"{OUTPUT_DIR}number_of_matches_graph.png")


def plot_locations_trajectory(ground_truth_locations, pnp_locations, bundle_adjustment_locations,
                              loop_closure_locations):
    fig, ax = plt.subplots(figsize=(19.2, 14.4))
    ax.scatter(x=ground_truth_locations[:, 0], y=ground_truth_locations[:, 2],
               label='Ground Truth', s=10, alpha=0.4, c='tab:blue')
...
def plot_PnP_median_projection_error_per_distance(pnp_mats):
...
def plot_BA_median_projection_error_per_distance(bundle_adjustment):
...
if __name__ == '__main__':
    # ================ arrange results from all stages ================ #
    ...
    db = DataBase(detector_type='SIFT')
    #stage 1 - PnP
    ...
    # ====== create graphs and statistics for [part 3] performance analysis ===== #
    ...
    plot_locations_trajectory(ground_truth_locations, pnp_locations, bundle_adjustment_locations,
                              loop_closure_locations)
    ...
    pnp_distances, pnp_links_num_per_distance = plot_PnP_median_projection_error_per_distance(pnp_mats)  # distance from last frame of track were we computed triangulation

    ba_distances, ba_links_num_per_distance = plot_BA_median_projection_error_per_distance(bundle_adjustment)  # distance from last frame of min(track, window) were we computed triangulation
    ...
    # relative BA estimation error of location and angle
